...
var arrowDetails = function () {
    for (var i = 0; i < item_base.length; i++) {
        if (item_base[i].name.match(/Arrow/g)) {
            ...
            var d = item.archery_damage, p = item.archery_speed, l = item.min_archery, c = item.archery_cooldown, c = c / 1000, x = item.archery_range, n = item_base[i].name, price = item.price;
